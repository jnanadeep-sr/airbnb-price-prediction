...
def regression_metrics_log_target(y_true_log, y_pred_log):
...
def print_metrics_log_target(name, y_true_log, y_pred_log):
    metrics = regression_metrics_log_target(y_true_log, y_pred_log)
    print(f"{name:<30} | R^2 Score: {metrics['r2']*100:.2f}% | MAE: ${metrics['mae']:.2f} | RMSE: ${metrics['rmse']:.2f}")
    return metrics
